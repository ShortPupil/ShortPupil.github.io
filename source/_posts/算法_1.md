...
1. 随机取数组的一个数num作为标准，将数分为三部分，小于放左边，等于放中间，大于放右边；
2. 然后将最后一个数和>num部分的第一个数进行交换，使得该数找到正确的位置，他的左边都是比他小的书，右边都是比他大的数
   - 实现：若比num小，当前位置和小于的最后一个位置+1的值交换，并当前位置++；若比num大，当前位置和大于的第一个位置-1的值交换；若等于num的值，当前位置++
3. 递归1

最优情况下O(nlogn)，最坏情况下O(n^2)，平均情况O（nlogn）
...
public class QuickSort {
	...
	public static void quickSort(int[] arr, int l, int r) {
		if (l < r) {
            /*随机快拍的随机是：随机选取一个数，和R进行交换，然后使用这个数作为num，所以每次选取的num是随机的，在计算时间复杂度时，就没有最优最差情况，结果是O(NlogN)*/
	...
	public static int[] partition(int[] arr, int l, int r) {
		...
		swap(arr, more, r);
        //返回等于的左右边界下标，通过下标确定小于区和大于区递归时的参数
